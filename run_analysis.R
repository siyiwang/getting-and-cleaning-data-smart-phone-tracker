...
measurements <- as.character(measurements[,2])
...
priorname <- colnames(completedata_sub)
colh <- colnames(completedata_sub)
colnames(completedata_sub)<- make.names(colh)
colnames(completedata_sub)<- gsub("[...]|[.]", "", colnames(completedata_sub))
postname <- colnames(completedata_sub)
# test dimension of completedata_sub, 68 columns extracted, only consider columns with mean() and std(), 
# but not meanFreq()＃dim(completedata_sub)： [1] 10299    68
...
write.table(completedata_final, file="./tidy_data.txt",row.name=FALSE)
